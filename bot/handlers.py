...
# =========================================================
# РАЗВЛЕКАТЕЛЬНАЯ КОМАНДА «МОЯ СТАТЬЯ»
# =========================================================
ARTICLE_RESULTS = [
    ("статье 158", "Кража — тайное хищение чужого имущества."),
    ("статье 159", "Мошенничество — похоже, кто-то слишком убедительно рассказывал истории."),
    ("статье 160", "Присвоение или растрата — чужое почему-то оказалось слишком близко."),
    ("статье 171", "Незаконное предпринимательство — бизнес-план явно ушёл не туда."),
    ("статье 171.2", "Незаконные организация и проведение азартных игр."),
    ("статье 180", "Незаконное использование средств индивидуализации."),
    ("статье 194", "Уклонение от уплаты таможенных платежей."),
    ("статье 199", "Уклонение от уплаты налогов и сборов организацией."),
    ("статье 205", "Террористический акт — слишком серьёзно для обычного дня."),
    ("статье 207", "Заведомо ложное сообщение об акте терроризма."),
    ("статье 210", "Организация преступного сообщества."),
    ("статье 213", "Хулиганство — порядок в чате явно пострадал."),
    ("статье 214", "Вандализм — кто-то опять оставил следы там, где не просили."),
    ("статье 222", "Незаконный оборот оружия."),
    ("статье 228", "Незаконный оборот наркотических средств."),
    ("статье 230", "Склонение к потреблению наркотических средств."),
    ("статье 242", "Незаконные изготовление и оборот порнографических материалов."),
    ("статье 264", "Нарушение правил дорожного движения."),
    ("статье 275", "Государственная измена — уровень приключений внезапно стал слишком высоким."),
    ("статье 280", "Публичные призывы к экстремистской деятельности."),
    ("статье 282", "Возбуждение ненависти либо вражды."),
    ("статье 286", "Превышение должностных полномочий."),
    ("статье 330", "Самоуправство — решил всё сделать по-своему."),
    ("статье 158.1", "Мелкое хищение — даже статья решила, что масштаб скромный."),
    ("статье 125", "Оставление в опасности."),
    ("статье 128.1", "Клевета — кто-то слишком много наговорил."),
    ("статье 137", "Нарушение неприкосновенности частной жизни."),
    ("статье 138", "Нарушение тайны переписки."),
    ("статье 139", "Нарушение неприкосновенности жилища."),
    ("статье 165", "Причинение имущественного ущерба без хищения."),
]

async def my_article(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Выдаёт случайную развлекательную «статью» пользователю."""
    import random
    ...
    name = _safe_text(user.first_name or user.username or "Участник", "Участник")
    article, description = random.choice(ARTICLE_RESULTS)

    await update.message.reply_text(
        f"⚖️ Сегодня <b>{name}</b> приговаривается к {article}. "
        f"{description}",
        parse_mode=ParseMode.HTML,
    )
